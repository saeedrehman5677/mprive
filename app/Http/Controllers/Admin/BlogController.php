...
class BlogController extends Controller
{
    ...
    public function store(StoreBlogRequest $request)
    {
        ...
        foreach ($request->input('detail_images', []) as $file) {
            $blog->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('detail_images');
        }
    ...
    public function update(UpdateBlogRequest $request, Blog $blog)
    {
        ...
        if (count($blog->detail_images) > 0) {
            foreach ($blog->detail_images as $media) {
                if (! in_array($media->file_name, $request->input('detail_images', []))) {
                    $media->delete();
                }
            }
        }
        $media = $blog->detail_images->pluck('file_name')->toArray();
        foreach ($request->input('detail_images', []) as $file) {
