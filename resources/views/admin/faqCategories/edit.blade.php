        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.faq-categories.update", [$faqCategory->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="name">{{ trans('cruds.faqCategory.fields.name') }}</label>
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $faqCategory->name) }}">
                    @if($errors->has('name'))
