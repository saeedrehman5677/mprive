...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} Subtype
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.subtypes.update', [$subtype->id]) }}">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label for="name">Name</label>
                    <input type="text" name="name" class="form-control" value="{{ old('name', $subtype->name) }}" required>
                </div>
                <div class="form-group">
